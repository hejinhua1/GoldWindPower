...
if __name__ == "__main__":
    ...
    checkpoint_prefix = 'TPALSTM_station{}_wind_'.format(station)
    log_path = "./logs/TPALSTM_station{}_wind_log".format(station)
    # 设置检查点路径和文件名前缀
    ...
    for epoch in range(resume_epoch + 1, epoch_size + 1):
        ...
        with torch.no_grad():
            ...
            for (hx, fx, y) in loop:  # hx: torch.Size([B, L, C]); hx: torch.Size([B, L-2, C]); y: torch.Size([B, L, C])
                ...
                y_raw, y_hat = normalizer.inverse_target(y_raw, y_hat, target='w', Norm_type=Norm_type)
                ...
                mape_loss = MAPE(y_hat, y_raw, normalizer.max_dict['rwind'])
